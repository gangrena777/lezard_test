...
$social_profiles = apply_filters( THEME_SLUG . '_social_share_links', array(
    'facebook', 'vk', 'twitter', 'ok', 'telegram', 'youtube', 'instagram',
    'tiktok', 'linkedin', 'whatsapp', 'viber', 'pinterest', 'yandexzen',
) );
    ...
    <?php
        foreach ( $social_profiles as $social_profile ) {
            ...
            if ( ! empty( $social_profile_link ) ) {
                ...
                if ( $social_profile == 'viber' ) {
                    if ( ! wp_is_mobile() ) {
                        $social_profile_link = 'viber://chat?number=' . $social_profile_link;
                    }
                    else {
                        $social_profile_link = str_replace( '+', '', $social_profile_link );
                        $social_profile_link = 'viber://add?number=' . $social_profile_link;
                    }
                }

                if ( $is_show_social_js ) {
                    if ( $social_profile != 'viber' ) $social_profile_link = base64_encode( $social_profile_link );
                    echo '<span class="social-button social-button__'. $social_profile .' js-link" data-href="' . $social_profile_link . '" data-target="_blank"></span>';
                } else {
